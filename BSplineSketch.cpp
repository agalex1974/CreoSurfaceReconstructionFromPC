...
void BSplineSketch::GetPerpendicularVector(const Pro3dPnt& normal, Pro3dPnt& outVector)
{
...
void BSplineSketch::NormalizeVector(Pro3dPnt& normal)
{
	double norm = sqrt(normal[0] * normal[0] + normal[1] * normal[1] + normal[2] * normal[2]);
	normal[0] /= norm; normal[1] /= norm; normal[2] /= norm;
}

void BSplineSketch::crossProduct(const Pro3dPnt& vect_A, const Pro3dPnt& vect_B, Pro3dPnt& cross_P)
{
...
void BSplineSketch::createGeometryAndSketch(const ProPoint3d& normal, const Pro3dPnt& point_on_plane, const std::vector<CPointEx3D>& threshold_pnts,
	const char* name)
{
	...
	std::string zeroPointNane = extrusionName + "_zero_Pnt";
	ProPoint3d zeroPoint;
	zeroPoint[0] = 0.0;
	zeroPoint[1] = 0.0;
	zeroPoint[2] = 0.0;

	zero = std::make_shared<CDatumPoint>(zeroPoint, zeroPointNane.c_str());

	std::string innerPointNane = extrusionName + "_Inner_Pnt";

	ProPoint3d inner_point;
	inner_point[0] = point_on_plane[0] + 0.1 * normal[0];
	inner_point[1] = point_on_plane[1] + 0.1 * normal[1];
	inner_point[2] = point_on_plane[2] + 0.1 * normal[2];
	...
	orientation.x = point_on_normal[0] - inner_point[0];
	orientation.y = point_on_normal[1] - inner_point[1];
	orientation.z = point_on_normal[2] - inner_point[2];
	...
	std::string orientationPlaneName = extrusionName + "_Orient_Plane";
	...
	cutting.x = point_on_normal[0] - inner_point[0];
	cutting.y = point_on_normal[1] - inner_point[1];
	cutting.z = point_on_normal[2] - inner_point[2];
	norm = sqrt(cutting.x * cutting.x + cutting.y * cutting.y + cutting.z * cutting.z);
	cutting.x /= norm;
	cutting.y /= norm;
	cutting.z /= norm;
	...
	createBSplineSketch(threshold_pnts, name);
}

BSplineSketch::BSplineSketch(const ProPoint3d& normal, const Pro3dPnt& inner_point, const std::vector<CPointEx3D>& threshold_pnts,
	int sketch_plane_id, const char* name):
	sketch_plane_id(sketch_plane_id)
{
	createGeometryAndSketch(normal, inner_point, threshold_pnts, name);
}
...
{
	std::string extrusionName(name);
	std::string sketchPlaneName = extrusionName + "_sktch";
	sketchPlane = std::make_shared<CDatumPlane>(normal, inner_point, sketchPlaneName.c_str());
	sketch_plane_id = sketchPlane->GetId();
	createGeometryAndSketch(normal, inner_point, threshold_pnts, name);
}

double BSplineSketch::distance(const CPointEx& pnt1, const CPointEx& pnt2)
{
...
ProError BSplineSketch::sketchSection(
	...
	/*     params [2] - vertical surface */
{
	...
	for (auto& poly_line : poly_lines) {
		...
		d[1] = poly_line[0].y - line1->end1[1];
		spline.point_arr[n][0] = line1->end1[0] + d[0];
		spline.point_arr[n][1] = line1->end1[1] + d[1];
...
ProError BSplineSketch::sketchSectionPhilipp(
	ProSection section,		/*In : the section */
	Parameter *params,
	int* extrusionId,
	int* orientation_plane_projection_id,
	...
	/*     params [2] - vertical surface */
{
	...
	for (int i = 0; i < projected_points.size(); i++)
	{
		...
		{
			if (!valid_points[j]) continue;
			if (distance(projected_points[i], projected_points[j]) < 5.5) // allow denser cloud
			{
				valid_points[j] = 0;
			}
		}
...
void BSplineSketch::createBSplineSketch(const std::vector<CPointEx3D>& pnts, const char* name)
{
	ProMdl model;
	...
	setDrawingFunction(sketchSection);
	//setDrawingFunction(sketchSectionPhilipp);
	Parameter params[3];
	...
	//FILE* file = fopen("sketchProcessBegin.txt", "w");
	//fprintf(file, "Sketching\n");
	//fclose(file);
	createSketch(model, params, &sketch_id, name, &orientation_id, &cutting_id);
	//file = fopen("sketchProcessEnd.txt", "w");
	//fprintf(file, "Done\n");
	//fclose(file);
