	...
	ProElement			*elem_tree)	    /* Out : element tree */
{
	...
	for (i = 0; i < n_elem; i++) {
		...
		if (elemarr[i].data.type != -1) {
			...
			{
				if (elemarr[i].data.v.r)
					ProSelectionToReference(elemarr[i].data.v.r, &ref_1);
				else {
					FILE* file = fopen("selectionNULL.txt","w");
					fclose(file);
					ProReferenceAlloc(&ref_1);
				}
...
ProError UtilityHelperClass::ProUtilModelitemNameGet(ProModelitem* modelitem, std::string& name)
{
	ProMdlName w_name;
	ProError err;
	char cname[255];
	err = ProModelitemNameGet(modelitem, w_name);
	if (err == PRO_TK_NO_ERROR)
	{
		ProWstringToString(cname, w_name);
...
ProError UtilityHelperClass::ProUtiGeomitemFromFeature(
	ProFeature *feature_axis,	    /* In: the feature */
	ProType type,		    /* In: Geomitem type */
	int  *featid)		    /*Out: Geomitem id  */
{
