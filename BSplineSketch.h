#ifndef BSPLINE_SKETCH
#define BSPLINE_SKETCH
#include <memory>
#include <vector>
#include <ProAxis.h>
...
class BSplineSketch : public ProSketchOnFlatSurface
{
public:
	/**
	 ...
	 */
	BSplineSketch(const ProPoint3d& normal, const Pro3dPnt& inner_point, const std::vector<CPointEx3D>& threshold_pnts, 
		const char* name);
	BSplineSketch(const ProPoint3d& normal, const Pro3dPnt& inner_point, const std::vector<CPointEx3D>& threshold_pnts,
...
private:
	...
	int sketch_plane_id;									/**< The sketch plane id */
	CPointEx3D normal;
	CPointEx3D orientation;
	CPointEx3D cutting;
	std::shared_ptr<CDatumPoint> zero;
	std::shared_ptr<CDatumPlane>	orientationPlane;
	std::shared_ptr<CDatumPlane>	sketchPlane;				/**< The plane on which the plane will reside */
	...
	void createGeometryAndSketch(const ProPoint3d& normal, const Pro3dPnt& inner_point, const std::vector<CPointEx3D>& threshold_pnts,
		const char* name);

	/**
	 * The sketching function
	 *
	 ...
	 */
	static ProError sketchSection(ProSection section, Parameter *params, int* extrusionId, int* orientation_plane_projection_id,
		int* cutting_plane_projection_id);
	static ProError sketchSectionPhilipp(ProSection section, Parameter *params, int* extrusionId, int* orientation_plane_projection_id,
		int* cutting_plane_projection_id);
	/**
	 ...
	 */
	void createBSplineSketch(const std::vector<CPointEx3D>& pnts, const char* name);
	/**
